...
describe('validateDataSheet', () => {
  ...
  it('returns empty array when all values are valid', () => {
    ...
    const errors = validateDataSheet(data, createSampleSpecs(), ['Color', 'Size']);
    expect(errors).toEqual([]);
  });
  ...
  it('ignores columns that do not match any specification', () => {
    ...
    const errors = validateDataSheet(data, createSampleSpecs(), ['Color', 'Material', 'Size']);
    expect(errors).toEqual([]);
  });
  ...
  it('handles specifications with no values', () => {
    ...
    const errors = validateDataSheet(data, specsWithEmpty, ['Color']);
  ...
  it('returns empty array when specifications is empty', () => {
    ...
    const errors = validateDataSheet(data, [], ['Color', 'Size']);
    expect(errors).toEqual([]);
  });
  ...
  it('handles whitespace in cell values', () => {
    ...
    const errors = validateDataSheet(data, createSampleSpecs(), ['Color']);
  ...
  it('detects error when spec value is deleted', () => {
    ...
    const errors = validateDataSheet(data, specsAfterDeletion, ['Color']);
...
describe('findDuplicateSKUs', () => {
  ...
  it('detects two rows with the same SKU', () => {
    ...
    expect(errors[0]).toEqual({
      row: 0,
      column: 0,
      message: 'Duplicate SKU "R-S" found in rows 0, 1',
      type: 'duplicate-sku',
    });
    expect(errors[1]).toEqual({
      row: 1,
  ...
  it('detects multiple rows with the same SKU', () => {
    const data: CellData[][] = [
      [cell('ABC'), cell('Red')],
      [cell('XYZ'), cell('Blue')],
      [cell('ABC'), cell('Red')], // Duplicate of row 0
      [cell('ABC'), cell('Red')], // Duplicate of rows 0 and 2
    ];
    const errors = findDuplicateSKUs(data);
    ...
    expect(errors[0].message).toBe('Duplicate SKU "ABC" found in rows 0, 2, 3');
  });
  ...
  it('detects multiple different duplicate SKUs', () => {
    ...
    expect(aaaErrors[0].row).toBe(0);
    expect(aaaErrors[1].row).toBe(2);
    // Check BBB duplicates
    const bbbErrors = errors.filter((e) => e.message.includes('BBB'));
    expect(bbbErrors).toHaveLength(2);
    expect(bbbErrors[0].row).toBe(1);
    expect(bbbErrors[1].row).toBe(3);
  });
  ...
  it('trims whitespace when comparing SKUs', () => {
    ...
    expect(errors).toHaveLength(2);
    expect(errors[0].message).toBe('Duplicate SKU "R-S" found in rows 0, 1');
  });
